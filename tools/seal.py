#!/usr/bin/env python3
"""Seal both credentials under a PIN, in the format source/Sealed.mc reads.
...
and inside the ciphertext, PKCS#7 padded to the block size:
    ...
    b"OTPM"  length:1 appPassword  length:1 otpPassword
...
Run directly to print a blob, which is how the test vector was produced:
    ...
    ./tools/seal.py --pin 1234 --app-password a --otp-password hunter2 \\
        --iterations 1000 --salt 000102...0f --iv 0f0e...00
"""
import argparse
import base64
import hashlib
...
def seal(app_password: str, otp_password: str, pin: str,
    ...
    if len(app) > 255 or len(otp) > 255:
        raise ValueError("a password longer than 255 bytes cannot be sealed")

    salt = salt if salt is not None else os.urandom(SALT_SIZE)
    iv = iv if iv is not None else os.urandom(IV_SIZE)

    body = MAGIC + bytes([len(app)]) + app + bytes([len(otp)]) + otp
...
def main() -> int:
    ...
    print(seal(args.app_password, args.otp_password, args.pin, args.iterations,
